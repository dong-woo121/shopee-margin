...
import { COUNTRY_MAP, PRODUCT_MAP } from '../constants';
...
export default function OrderCalculator({ country, mode, settings }: Props) {
  ...
  const { items, settlementLocal, addItem, removeItem, updateSalePrice, setSettlementLocal, clearOrder } = useOrderItems(country);
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <div className="mx-4 my-3 flex items-center justify-between bg-orange-50 rounded-xl px-4 py-2.5">
        ...
        <span className="text-xs text-gray-400">매입률 {settings.costRate}%</span>
      </div>
      ...
      {items.length === 0 && (
        <div className="mx-4 mb-4 rounded-2xl bg-gray-50 border border-dashed border-gray-200 p-6 text-center">
          <p className="text-sm text-gray-400">위에서 제품을 탭해<br />주문을 구성하세요</p>
        </div>
      )}
